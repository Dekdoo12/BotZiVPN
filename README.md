...
## Installasi Otomatis
```bash
sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update -y && apt install -y git && apt install -y curl && curl -L -k -sS https://raw.githubusercontent.com/Dekdoo12/BotZiVPN/main/start -o start && bash start sellzivpn && [ $? -eq 0 ] && rm -f start
```
## UPDATE
```bash
curl -sSL https://raw.githubusercontent.com/Dekdoo12/BotZiVPN/main/update.sh -o update.sh && chmod +x update.sh && bash update.sh
```
